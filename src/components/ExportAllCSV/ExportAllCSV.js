...
class ExportAllCSV extends Component {
  ...
  getRows = (headers) => {
    const { sessionForms, dataForms, selectedRows, answerSets, projects, project_id } = this.props;
    ...
    (process.env.REACT_APP_BATEMAN_BUILD === 'true') &&
      forms.forEach(f => {
        const template = JSON.parse(f.template_json);
        ...
        if (hasSpeciesCode) {
          ...
          const speciesAnswerSet = answerSet && JSON.parse(answerSet.answers);
    ...
    const rows = selectedRows.map(entry => {
      const json = (entry.entry_json) ? JSON.parse(entry.entry_json) : JSON.parse(entry.session_json);
